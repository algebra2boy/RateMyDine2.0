 ...
 * const <collection-name>: Collection<T> = MongoDB.getCollection(<collection-name>);
 * ```
 */
export class MongoDB {
  ...
  public static getRateMyDineDB(): Db {
    if (!MongoDB.rateMyDineDB) throw new Error('Mongo ratemyDine Database does not exist yet...');
    return MongoDB.rateMyDineDB;
  }

  /**
   * This static method retrieves the a collection with a name
   * @returns MongoDb Collection.
   */
  public static getCollection(name: string): Collection {
    if (!MongoDB.rateMyDineDB) throw new Error('Mongo ratemyDine Database does not exist yet...');
    return MongoDB.getRateMyDineDB().collection(name);
  }
